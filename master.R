...
server <- function(session, input, output) {
  rv <- reactiveValues(page = 1, studentLogInSuccess=FALSE, facultyLogInSuccess=FALSE, 
                       adminLogInSuccess = FALSE, loginDecisionSuccess= FALSE, errors = rep(FALSE, 8))
    ...
    if (length(prof_courses)>0) {
      ...
      output$tabs1 <- renderUI({
        ...
        tabs[[length(prof_courses)+1]] <- tabPanel("Summary", 
                                                   ...
                                                   actionButton("submitFaculty", "Submit")
        )
  ...
  observeEvent(input$loginDecision, {
    tryCatch({
      ...
      student_data <<- student_info[input$usernameDecision == student_info$netid & input$pinDecision == student_info$pin, ]
